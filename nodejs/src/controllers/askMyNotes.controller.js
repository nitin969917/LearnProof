...
/**
 * AI Chat using RAG on subject notes
 */
const chatWithNotes = async (req, res) => {
    try {
        ...
        // 1. Re-phrase query for better retrieval (Fast check with Cerebras)
        let searchQuery = query;
        if (recentHistory.length > 0) {
            try {
                const rephraseResponse = await cerebras.chat.completions.create({
                    model: 'llama3.1-8b',
                    messages: [
                        { role: 'system', content: 'You are a search assistant. Convert the user\'s message and conversation history into a standalone search query to find relevant information in their notes. Return ONLY the search query string.' },
                        { role: 'user', content: `History:\n${historyText}\n\nLast Message: ${query}\n\nStandalone Search Query:` }
                    ],
                    temperature: 0
                });
                searchQuery = rephraseResponse.choices[0].message.content.trim();
                console.log(`[RAG] Re-phrased query: "${query}" -> "${searchQuery}"`);
            } catch (rephraseErr) {
                console.warn('[RAG] Query re-phrase failed, using original.');
            }
        }

        // 2. Get relevant snippets from vector store with increased topK (7)
        const results = await ragService.querySubjectIndex(subject.id, searchQuery, 7);
        
        if ((!results || results.length === 0) && recentHistory.length === 0) {
            // Trigger auto-recovery check if results are empty but notes exist
            const notes = await prisma.subjectNote.findMany({ where: { subjectId: subject.id } });
            if (notes.length > 0) {
                console.log(`[RAG-Auto] Empty results for ${subject.name} but notes exist. Triggering recovery check.`);
                notes.forEach(note => {
            ...
            const fallback = `Not found in your notes for ${subject.name}.`;
            ...
            return res.json({ response: fallback, citations: [], confidence: 0 });
        }
        ...
        results.forEach(([doc, score], i) => {
            ...
            contextText += `---\nSource Material ${i+1} (From ${filename}, Page ${pageNum}):\n${snippet}\n`;
            citations.push({
                filename,
                page: pageNum,
        ...
        avgScore = (results.length > 0) ? (avgScore / results.length) : 0.5;
        const confidence = Math.round(avgScore * 100);

        // 4. Call LLM (Cerebras Llama 3.1 8B)
        let finalAnswer = "";
        let retryCount = 0;
        const maxRetries = 2;
            ...
            try {
                const completion = await cerebras.chat.completions.create({
                    ...
                    messages: [
                        { 
                            role: 'system', 
                            content: `You are a strict "Document-Only" study assistant for the subject '${subject.name}'. 

CRITICAL RULES:
1. ANSWER ONLY FROM CONTEXT: You must answer the student's question ONLY using the provided "Context Snippets" or "Conversation History".
2. NO EXTERNAL KNOWLEDGE: Do not use any outside knowledge or information not contained in the provided snippets. If the information is not there, you MUST say you don't know based on their notes.
3. STRICT FALLBACK: If the answer is not present in the context OR history, you MUST respond exactly with: "Not found in your notes for ${subject.name}." 
4. NO GENERAL CHITCHAT: If the user asks general knowledge questions (e.g., "Who is the president?" or "Capital of France") that are not in the notes, use the STRICT FALLBACK.
5. FORMATTING: If the user asks for a specific format (e.g., "10 points"), follow it, but still only using the note's content.` 
                        },
                ...
                console.error(`[AskMyNotes] Cerebras Attempt ${retryCount + 1} failed:`, llmError.message);
                if (llmError.message.includes('429')) {
        ...
        await prisma.askMyNoteChat.create({
            data: {
                ...
                confidence: finalAnswer.includes(`Not found in your notes for ${subject.name}`) ? 0 : confidence
            }
        });

        res.json({
            response: finalAnswer,
            citations: finalAnswer.includes(`Not found in your notes for ${subject.name}`) ? [] : citations,
            confidence: finalAnswer.includes(`Not found in your notes for ${subject.name}`) ? 0 : confidence
        });

    } catch (error) {
        console.error('[AskMyNotes] Chat detailed error:', {
            message: error.message,
            stack: error.stack,
            type: error.constructor.name
        });
        const status = error.message.includes('429') ? 429 : 500;
        const message = error.message.includes('429') ? 'AI is currently busy. Please try again in 30 seconds.' : 'Error communicating with AI';
