...
import ParticipantMeetingEndedModal from './ParticipantMeetingEndedModal.jsx';
...
function CustomLanguageRoomContent({ roomName, handleLeaveRoom, user, dbRoom, userIdentity, isRestoring, hasExplicitlyLeft, onHostEndSession, onParticipantEnded }) {
  ...
  const handleHostEndedMeeting = useCallback((customMsg) => {
    ...
    // Explicitly flag departure so PiP window never activates
  ...
  const handleConfirmEndRoom = async () => {
    ...
    // 1. Immediately flag explicit leave so PiP NEVER triggers
    if (hasExplicitlyLeft) {
      hasExplicitlyLeft.current = true;
    }
    const pip = useLiveRoomPipStore.getState();
  ...
  useEffect(() => {
    ...
    const fetchTokenAndRoom = async () => {
      ...
      try {
        ...
        // If room no longer exists in DB, it was ended by host — show friendly ended modal for participants
        if (!roomInfo) {
          const userIdentifier = user?.id || user?.uid;
          const isRoomHost = dbRoom && userIdentifier && String(dbRoom.creatorId) === String(userIdentifier);
          if (!isRoomHost) {
