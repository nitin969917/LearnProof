...
const ProfileModal = ({ isOpen, onClose }) => {
    ...
    const [socialProfile, setSocialProfile] = useState(() => useSocialFeedStore.getState().socialUser || null);

    // Initial check: if we already have data from store, user, or cache, do not show empty state
    const hasExistingData = Boolean(socialProfile || socialUser || user || profileData);
    const [loading, setLoading] = useState(!hasExistingData);

    // Escape key listener to close smoothly
    useEffect(() => {
        if (!isOpen) return;
    ...
    useEffect(() => {
        ...
        // Keep local socialProfile in sync with latest store
        const currentStoreUser = useSocialFeedStore.getState().socialUser;
        if (currentStoreUser) {
            setSocialProfile(currentStoreUser);
        }

        // 1. Fetch latest Social Hub profile in background (stale-while-revalidate)
        ...
        // 2. Fetch academic/gamification stats in background
        const fetchProfileData = async () => {
            ...
            try {
                ...
                if (res.data) {
                    ...
                    try {
                        localStorage.setItem(PROFILE_CACHE_KEY, JSON.stringify(res.data));
                    } catch (e) {}
                }
            } catch (err) {
        ...
        <AnimatePresence>
            {isOpen && (
                <motion.div
                    ...
                    className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm flex justify-center items-center z-[100] p-4 will-change-[opacity]"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                ...
                >
                    <motion.div
                        ...
                        className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl max-w-sm w-full overflow-hidden relative border border-slate-200/80 dark:border-slate-800 will-change-[transform,opacity]"
