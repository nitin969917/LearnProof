          ...
          <div className="bg-white dark:bg-gray-900 flex-1 min-w-0 transition-colors duration-200">
            <div className="max-w-5xl mx-auto p-4 sm:p-6 pb-20 lg:pb-6">
              ...
              <div className="mt-4">
                ...
                <div className="py-6">
                  {/* Playlist Tab (Mobile Only) */}
                  {activeTab === 'playlist' && playlist && (
                    <div className="lg:hidden space-y-1 divide-y divide-gray-50 dark:divide-slate-800/50">
                      {(playlist.videos || []).map((v, index) => {
                        const isActive = v.vid === video.vid;
            ...
            {(playlist.videos || []).map((v, index) => {
              ...
              return (
                <div
                  ...
                  ref={isActive ? activeVideoRef : null}
                  onClick={() => navigate(`/classroom/${v.vid}`)}
                  className={`group flex gap-3 p-3 cursor-pointer transition-all duration-200 ${
                    isActive
                ...
                >
                  ...
                  {/* Info */}
                  <div className="flex-1 min-w-0">
                    <h3 className={`text-xs font-bold line-clamp-2 mb-1.5 leading-snug transition-colors ${
                      isActive ? 'text-orange-600 dark:text-orange-400' : 'text-gray-700 dark:text-slate-300 group-hover:text-orange-500'
                    }`}>
                      {v.name}
                    </h3>
                    {/* Progress bar */}
                    <div className="h-1 w-full bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
                      <div
                        ...
                        className={`h-1 rounded-full transition-all duration-500 ${
                          v.is_completed ? 'bg-green-500' : 'bg-gradient-to-r from-orange-500 to-amber-400'
                        }`}
                      ></div>
                    </div>
